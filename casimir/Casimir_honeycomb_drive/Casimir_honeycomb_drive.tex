...
\def\myemail{me@somewhere.com}
\def\ukrvisnyk{Вісник ХНУ імені В.Н. Каразіна, серія «Фізика», вип. 38, p. 30-38 (2023)}
\def\envisnyk{The Journal of V.N. Karazìn Kharkiv National University. Series “Physics” Iss. 38, p. 15-20 (2023)}
\def\myworkplace{V.N. Karazin Kharkiv National University, 61022, Svoboda Sq. 4, Kharkiv, Ukraine}
...
tmargin=1.2cm,bmargin=3cm,lmargin=2cm,rmargin=2cm}
...
\renewcommand{\headrulewidth}{0pt} % remove the top horizontal rule
...
\section*{\centering{\textbf{\mytitle}}}

\centerline{{\myname \,\orcidlink{\myorcidlink} }}
\vspace{3mm}
...
\begin{keywords}
Two dimensional one body Casimir effect, nanocells, nanohoneycomb, Casimir thrust, Casimir driving force
\end{keywords}
...
Antipin A. V. \cite{Antipin2012} expects appearance of the driving force (due to the one body
Casimir effect) as a result of the difference of impact of the virtual particles (photons) on the
external and internal reflective surfaces of pyramidal, conical or corner shape.
...
% \end{wrapfigure}
    ...
    \section*{\centering{Two dimensional Casimir's
...
\(E = \frac{\hbar\,c}{2}\int\limits_{0}^{\infty}\left[{\sqrt{k_z^2}+2\sum\limits_{n_x=1}^{\infty}\sum\limits_{n_y=1}^{\infty}\sqrt{\frac{n_x^2 \pi^2}{a^2}+\frac{n_y^2 \pi^2}{a^2}+k_z^2}}\right]d{n_z}\).
    ...
    In consideration of \(dn_z = \frac{L/2}{\pi}\,dk_z\) we can find the specific energy density \(E/V\), where
\(V = V_{small} = a^2 L/2\):
...
continuous variables. Thus, for large cavity between pure plane and \(xy\) face we find
\noindent
\(\begin{array}{c}
\begin{array}{ll}
...
integral and it is therefore easily seen that energy of large cavity is given by
...
\(dn_z = \frac{L/2}{\pi}\,dk_z\),
    ...
    Now for the specific energy density \(E/V\) for large cavity, where
...
And finally for large cavity between pure plane and \(xy\) face we can write the following energy density formula
...
but for small cavity, one honeycomb cell we have
\noindent
...
Therefore, it is easy to see that the interaction energy is determined by the following energy density difference:
%    \[\begin{array}{lr}
%\delta\,\frac{E}{V} =
%\begin{array}{c}
...
\section*{\centering{Euler--Maclaurin 2D formula}}\label{eulermaclaurin-2d-formula}
\vspace{-3.5mm}
...
we see that it consists of 4 parts:
...
the following summands which can be represented in the table form:
...
\end{array}\end{equation}
    ...
    And taking into account that we have function \(G\) which is symmetric on
its \(n_x\) and \(n_y\) arguments, so above two dimentional
Euler--Maclaurin marix is symmetric too.

% \end{multicols}

    \section*{\centering{Summary of Euler--Maclaurin
...
\end{array}\)
    ...
    So we can find expression (\ref{eq:12}) using the following summation:
...
\noindent
    ...
    are 0 in sum. And so any possible non zero result of expression (\ref{eq:12}) should be related with
remainder part:
...
as \(a\,k_m\,>>\,1\). But now we can investigate how the expression (\ref{eq:12}) will depends on \(a k_m\)
...
\caption{}{\centering{\textbf{Fig.2.} How $\delta\left(E/V\right)$ depends on $a k_m$}}
\label{fig:G_on_a_km}
\end{figure}
...
configuration? Investigation of this question in appendixes B and C
shows that \(F/S \approx \delta\left(E/V\right)\).

% \end{multicols}

    \section*{\centering{CONCLUSION}}\label{conclusions}
\vspace{-3.5mm}
...
\end{multicols}
    ...
    \section*{\centering{Appendix A. Integration details}}\label{appendix-a.-integration-details}
\vspace{-3.5mm}
...
\end{multicols}
    ...
    \section*{\centering{Appendix B. Electromagnetic pressure
...
% \end{multicols}
    ...
    \section*{\centering{Appendix C. Hamiltonian mechanics
...
\(E = \frac{\hbar c}{2}\int\limits_{0}^{\infty}\left[{\sqrt{k_z^2}+2\sum\limits_{n_x=1}^{\infty}\sum\limits_{n_y=1}^{\infty}\sqrt{\frac{n_x^2\pi^2}{a^2}+\frac{n_y^2\pi^2}{a^2}+k_z^2}}\right]d{n_z}\).
    ...
    In consideration of \(dn_z = \frac{l}{\pi}\,dk_z\) we can find the specific energy density \(E/S\), where
\(S = S_{small} = a^2\):
...
\(dn_z = \frac{L-l}{\pi}\,dk_z\).
    ...
    Now for the specific (per area) energy density \(E/S\) for large cavity, where
...
to perfectly conducting honeycomb on a plate \({F}/{S}\) is the same as formula (\ref{eq:2})
...
\begin{multicols}{2}
    ...
    \section*{\centering{Appendix D. The derivation of the thrust formula for the
...
4, the thrust of one square {\glqq}opened{\grqq} cell will be equal to
