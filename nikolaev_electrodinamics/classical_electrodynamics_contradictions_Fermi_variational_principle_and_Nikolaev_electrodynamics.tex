...
\begin{enumerate}
    ...
    \item Если бы $\mu_{\parallel}$ зависела от ускорения, волновое уравнение стало бы нелинейным и непредсказательным.
...
\subsection*{Как возникает поправка Ферми из $\mu_{\parallel}$}

Рассмотрим действие взаимодействия в модифицированной теории:

\begin{equation}
S_{int} = -\frac{1}{c} \int j^\mu A_\mu \, d^4x
\label{eq:action_standard_fermi}
\end{equation}

В теории с $\mu_{\parallel} \neq \mu$ пропагатор фотона (функция Грина) модифицируется. Для продольной компоненты:

\begin{equation}
D_{\parallel}(k) = \frac{1}{k^2 - (\mu - \mu_{\parallel})\frac{\omega^2}{c^2}}
\label{eq:propagator}
\end{equation}

В координатном пространстве это приводит к тому, что эффективное действие для ускоряющегося заряда приобретает вид:

\begin{equation}
S_{eff} = S_{classical} + \delta S_{longitudinal}
\label{eq:action_eff}
\end{equation}

где $\delta S_{longitudinal}$ при разложении по $v/c$ даёт член:

\begin{equation}
\delta S_{longitudinal} \propto \frac{\mu - \mu_{\parallel}}{\mu} \int \frac{\vec{a} \cdot \vec{r}}{c^2} \, dt
\label{eq:action_longitudinal}
\end{equation}

Сравнивая с поправкой Ферми:

\begin{equation}
\delta S_{Fermi} = \int \frac{\vec{a} \cdot \vec{r}}{c^2} \, dt
\label{eq:action_fermi}
\end{equation}

Получаем связь:

\begin{equation}
\frac{\mu - \mu_{\parallel}}{\mu} = \xi_F \quad \Rightarrow \quad \mu_{\parallel} = \mu (1 - \xi_F)
\label{eq:mu_fermi_link}
\end{equation}

где $\xi_F$ — безразмерный параметр, определяемый из требования воспроизведения проблемы 4/3 (для сферы $\xi_F = 1/3$).
...
\begin{itemize}
    ...
    \item \textbf{Различие $\mu \neq \mu_{\parallel}$} приводит к тому, что при ускорении заряда возникает дополнительная сила, которая в пределе точечного заряда воспроизводит поправку Ферми.
\end{itemize}

Таким образом, \textbf{$\mu_{\parallel}$ — это фундаментальное свойство вакуума}, а поправка Ферми — её проявление на уровне движения зарядов.
