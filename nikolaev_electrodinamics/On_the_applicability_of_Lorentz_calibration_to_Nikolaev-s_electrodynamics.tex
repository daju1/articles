...
\author{А.Ю.Дроздов}
    ...
    % Document parameters
    % Document title
    \title{О применимости калибровки Лоренца к электродинамике Николаева}
\date{2019..2020}
...
\section*{Электрический импульс центрально симметричного взрыва плазмы}
    

    
    Одной из нерешённых проблем современной электродинамики является
проблема электромагнитного импульса ядерного взрыва. В журнале
Инженерная физика появилась статья \cite{Mende2013} в
...
предполагает зависимость скалярного потенциала заряда от скорости.

Такая зависимость была получена из анализа законов индукции электрического
...
включающих преобразования Галилея.

Приводятся также косвенные
...
постоянного тока \cite{Edwards1976} \cite{Roser1962} \cite{Baker1964} \cite{Mende1993}.

В 2015 году появилась публикация \cite{Mende2015} с описанием
...
Для начала следует отметить, что попытка
...
случая случае центрально-симметричного движения зарядов.

Действительно, пусть мы имеем три ИСО: первая неподвижная, вторая движется со скоростью
\(dv\), а третья со скоростью \(2dv\). В третьей ИСО расположен
заряженный стержень. Во второй появляется прибавка магнитного поля
\(dB\). А в первой прибавка электрического поля \(dE\).
...
скоростями очень много.

В случае разогретой плазмы, имеющей сферическую
...
движется со скоростью \(-2dv\).

Таким образом для разбора ситуации нам
...
прибавка электрического поля \(-dE\).
...
суммарная прибавка электрического поля равна нулю.

Причина такого вывода
...
применима к сферически симметрическому электрическому вибратору.


Кроме того нужно иметь ввиду, что при выводе формулы скалярно-векторного
...
потенциала движущегося точечного заряда неверна.

Желающий уточнить формулу скалярно-векторного потенциала, учитывая лишь только
...
электрического поля учесть также и продольный согласно \cite{Nikolaev2004} механизм
образования прибавки Е, то в результате формула
...
\section*{Расчёт задачи центрально-симметричного электрического вибратора}

Взятая за основу данного расчёта идея происхождения электромагнитного импульса
...
магнитное поле) и далее - продольную волну».

Для решения задачи вычисления поля центрально-симметричного взрыва, был использован метод,
применённый Таммом в § 98 «Осциллятор. Запаздывающие потенциалы поля
осциллятора» \cite{Tamm1957}
...
\begin{equation}\varphi \left( \theta ,\phi ,r,t
\right)=\frac{1}{\varepsilon }\int{\frac{\rho \left( \theta ',\phi ',r',t' \right)}{R}}~dV'\end{equation}
\begin{equation}\overrightarrow{{{A}_{B}}}\left( \theta ,\phi ,r,t
\right)=\frac{\mu }{c}\int{\frac{\overrightarrow{j}\left( \theta ',\phi ',r',t' \right)}{R}}~dV'\end{equation}
...
\begin{equation}\overrightarrow{B}=\mu ~\overrightarrow{H}\end{equation} постольку и
...
\begin{equation}\overrightarrow{R}=\overrightarrow{{{R}_{O}}}-\overrightarrow{R'}\end{equation}
Где \begin{equation}\overrightarrow{R'}\end{equation} - есть расстояние от точки взрыва \(Q\) до
центра взрыва \(O\). Раскладывая выражение для \(|\overrightarrow{R}|\)

\begin{equation}
R = |\overrightarrow{R}|=\sqrt{{{R}_{O}}^{2}-2\overrightarrow{{{R}_{O}}}\overrightarrow{R'}+R{{'}^{2}}}
\end{equation}

в ряд Тейлора и пренебрегая вторыми и старшими степенями
\({{R}^{'}}\), получаем
\begin{equation}R\approx {{R}_{O}}-\frac{\overrightarrow{{{R}_{O}}}\overrightarrow{R'}}{{{R}_{O}}}\end{equation}
Далее, согласно \cite{Tamm1957} выражая в подынтегральных
выражениях для скалярного и векторного потенциалов \(R\) через
\({{R}_{O}}-\frac{\overrightarrow{{{R}_{O}}}\overrightarrow{R'}}{{{R}_{O}}}\),
...
\begin{equation}\int{\rho \left( \theta ',\phi ',r',t-\frac{{{R}_{O}}}{c} \right)}~dV'\end{equation}
с множителем \({}^{1}/{}_{{{R}_{O}}}\) , вынесенным за знак интеграла, а
второй интеграл \(
-\frac{\overrightarrow{{R}_{O}}}{{R}_{O}}\frac{\partial }{\partial {{R}_{O}}}
\left\{
...
\right\}\)
равен дивергенции, взятой по точке наблюдения вектора Герца
системы \(
-di{v}\emph{{a}}
\left\{
\frac{\overrightarrow{p}\left( t-\frac{{R}_{O}}{c} \right)}{{R}_{O}}
\right\}
\). В контексте решаемой задачи оба этих интеграла равны нулю.
...
слагаемое\begin{equation}\frac{1}{c{{R}_{O}}}\int{\overrightarrow{j}\left( \theta ',\phi ',r',t-\frac{{{R}_{O}}}{c} \right)}~dV'\end{equation},
являющееся производной по времени вектора Герца системы
\begin{equation}\frac{1}{c{{R}_{O}}}\frac{\partial }{\partial t}p\left(
t-\frac{{{R}_{O}}}{c} \right)\end{equation}. Но вот второе слагаемое, отброшенное
...
\begin{equation}\int{\frac{\overrightarrow{j}\left( \theta ',\phi ',r',t-\frac{R}{c} \right)}{R}}~dV'\approx -\int{\frac{\overrightarrow{R'}\overrightarrow{{{R}_{O}}}}{{{R}_{O}}}\frac{\partial }{\partial {{R}_{O}}}\frac{\overrightarrow{j}\left( \theta ',\phi ',r',t-\frac{{{R}_{O}}}{c} \right)}{{{R}_{O}}}\ dV'}\end{equation}
...
Интеграл
\begin{equation}
\int{\overrightarrow{R'}\cdot \overrightarrow{j}\left( \theta ',\phi ',r',t-\frac{{{R}_{O}}}{c} \right)}~dV'
=s\left(t-\frac{{{R}_{O}}}{c} \right)
\end{equation} Представляет собой величину, которую мы
назовём скалярным магнитным моментом \(s\) центрально-симметричной системы
зарядов в момент \(t-\frac{{{R}_{O}}}{c}\) Таким образом векторный
потенциал равен

\begin{equation}\overrightarrow{{{A}_{H}}}=-\frac{1}{c}\frac{\overrightarrow{{{R}_{O}}}}{{{R}_{O}}}\frac{\partial }{\partial {{R}_{O}}}\left\{
\frac{s\left( t-\frac{{{R}_{O}}}{c} \right)}{{{R}_{O}}} \right\}
\end{equation}

Это выражение можно в окончательной форме записать как дивергенцию в точке
наблюдения
\begin{equation}
\overrightarrow{{A}_{H}}=-\frac{1}{c}\frac{\overrightarrow{{R}_{O}}}{{R}_{O}}
di{{v}}_{a}\left\{
\frac{s\left( t-\frac{{{R}_{O}}}{c} \right)}{{R}_{O}}
\right\}
\end{equation}
...
произведения
\(\frac{\overrightarrow{R'}\overrightarrow{{{R}_{O}}}}{{{R}_{O}}}\)
на вектор
...
операция была бы математически допустима.


А поскольку мы исследуем
...
\begin{equation}{{A}\emph{{B,{{R}}{0}}}}\left( r,t
...
\end{equation}
где \({{j}\emph{{{{R}}{0}}}}\) по сути это проекция тока переноса на ось \(z\)
...
\theta ' \right)
\end{equation}
Теперь уравнение 110 будет выглядеть так
\begin{equation}
-\frac{\overrightarrow{{{R}_{O}}}}{{{R}_{O}}}\frac{\partial }{\partial {{R}_{O}}}\left\{
...
\section*{Расчёт ЭМИ центрально-симметричного взрыва на основе скалярного
магнитного момента}

Выделим условно четыре периода развития взрыва:
...
ускорение частиц одного и того же типа одинаковы.

Для расчёта скалярного
...
координат. Скалярный магнитный момент

\begin{equation}s\left( t' \right)=\int{\overrightarrow{R'}\cdot \overrightarrow{j}\left( \theta ',\phi ',r',t' \right)}~dV'=\int{r'\cdot {{j}_{r}}\left( r',t' \right)}~4\pi r{{'}^{2}}dr'
\end{equation}
...
Эта компонента электрического поля возникает благодаря закону
...
=
\int\limits_{0}^{\infty }
r'\cdot {{j}_{r}}\left( r',t-\frac{{R}_{O}}{c} \right)
\, 4\pi r{{'}^{2}}dr' \\
s\left( t-\frac{{{R}_{O}}}{c} \right)
=
e\int\limits_{0}^{\infty }{r'\cdot n\left( r',t-\frac{{{R}_{O}}}{c} \right)}\,
\cdot {{v}_{r}}\left(r',t-\frac{{{R}_{O}}}{c} \right)
\,4\pi r{{'}^{2}}dr' \\
\end{equation}
...
существенный вклад в
 ...
 \begin{equation}\frac{\partial }{\partial t}s\left(
t-\frac{{{R}_{O}}}{c} \right)\end{equation} вносит рост скорости частиц. Теперь
...
При расширении облака частиц концентрация

 \begin{equation}n\left(
r',t-\frac{{{R}_{O}}}{c} \right)\end{equation}убывает с квадратом \begin{equation}r'\end{equation}. Но в
подынтегральное выражение входит \begin{equation}r{{'}^{3}}\end{equation}, чем
обусловлена положительная величина

\begin{equation}\frac{\partial }{\partial t}s\left( t-\frac{{{R}_{O}}}{c} \right)\end{equation}.
...
\begin{equation}\frac{\partial }{\partial {{R}_{O}}}\end{equation}в выражении для скорости
изменения векторного потенциала

\begin{equation}-~c\frac{{{\left( \partial {{A}_{H}} \right)}_{R}}}{\partial t}=\frac{\partial }{\partial {{R}_{O}}}\left\{
\frac{1}{{{R}_{O}}}\frac{\partial }{\partial t}s\left(
t-\frac{{{R}_{O}}}{c} \right) \right\}\end{equation} Дифференцируем
...
\begin{equation}
-c\frac{{{\left( \partial {{A}_{H}} \right)}_{R}}}{\partial t}
=-\frac{1}{{{R}_{O}}^{2}}\frac{\partial }{\partial t}\int\limits}{0}}{\infty }{r'\cdot {{j}_{r}}\left(
r',t'
...
Пренебрегая эффектами запаздывания
потенциала, получаем
...
\textit{Apfelbaum, E. a. (2009). The predictions of the critical point parameters for Al, Cu and W found from the correspondence
...
\bibitem{Baker1964}
\textit{Baker, D. A. (1964). Second-Order Electric Field due to a Conducting
Curent. American Journal of Physics, 32(2), 153-157.}

\bibitem{Edwards1976}
\textit{Edwards, W. F. (1976). Continuing investigation into possible electric arising from
steady conduction current. Phys. Rev. D, 14(4), 922-\/-938.
doi:10.1103/PhysRevD.14.922
}


\bibitem{Mende1993}
\textit{Mende, F. F. (1993). Conference ``Physics in Ukraine''. Experimental corroboration and theoretical interpretation of dependence of charge value on DC flow velocity through superconductors.
Kiev.
}

\bibitem{Roser1962}
\textit{Roser, W. (1962). Second-Order Electric Field due to a Conducting
Curent. American Journal of Physics, 30(7), 509-511.
}

\bibitem{Adamyan1999}
\textit{Адамьян Ю.Э., В. В. (1999). Нагрев и ускорение плазмы при взрыве проводника в вакууме в
сильном магнитном поле. Журнал технической физики, 69(5).
}

\bibitem{Dobrovolskaya2015}
\textit{Добровольская А.С., К. Н. (2015). Применение дискретной кинетической модели для
...
http://chemphys.edu.ru/issues/2015-16-3/articles/571/
}

\bibitem{Zinoviev1989}
\textit{Зиновьев, В. (1989). Теплофизические свойства металлов при высоких температурах.
«Металлургия».
}

\bibitem{Lifshic1973}
\textit{Лифшиц, Л. Л. (1973). Теоретическая физика. В Теория поля
(Т. 2). Москва.
}

\bibitem{Mende2013}
\textit{Менде, Ф. Ф. (2013). Электрический импульс космического
термоядерного взрыва. Инженерная физика, 16-24.
}

\bibitem{Mende2015}
\textit{Менде, Ф. Ф. (2015). Является ли заряд инвариантом скорости? Получено из
http://fmnauka.narod.ru/javljaetsja\_li\_zarjad\_invariantom\_skorosti.pdf
}

\bibitem{Nikolaev2004}
\textit{Николаев, Г. (2004). Электродинамика физического вакуума. Томск: НТЛ.
}

\bibitem{Sasakin2016}
\textit{Сасакин, М. (2016). Разлёт в вакуум и взаимодействие с преградой
мелкодисперсных частиц при электрическом взрыве проводника. Санкт-Петербург.
}
