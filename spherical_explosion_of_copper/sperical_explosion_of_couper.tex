...
Он применяет Гамильтонов метод с использованием аппарата теории относительности следующим образом: для вычисления действия нужно использовать переменные границы интегрирования для времени t1 и t2.

Я не уверен, что Ферми решил эту проблему до конца правильно поскольку в этой его статье от 1923 года я нашел пару существенных ошибок, а именно:

Я вчитался в его формулы. И для себя отметил три сомнительных момента.

Первый. Непонятно почему он находит возможным пренебречь последним интегралом (в третьей без номера формуле сверху на странице 80).

Второе. Он пишет, что E(i) это сумма сил Кулона. А вот нет. Если разобраться как Батыгин и Топтыгин вычисляют инертную массу (а я разобрался. это задача 689 их задачника 1962 года издания), то E(i) это градиент скалярного потенциала минус производная по времени векторного, причем оба эти потенциала нужно брать с запаздыванием, и если даже не брать поле по Лиенару Вихерту как Батыгин с Топтыгиным, то хотя бы Лагранжиан Дарвина применить (а лучше до третьего порядка) , а не просто лишь кулоновское поле.
...
Я пытался вычислять инертную массу электрона с помощью потенциалов ЛВ и получал вслед за Батыгиным-Топтыгиным 4/3
...
Исходя из формализма ЛВ этот дополнительный интеграл Ферми как раз равен половине градиентного интеграла который возникает при решении задачи об инертной массе электрона методом ЛВ. А градиентный интеграл это интеграл поля $Е_1$ в обозначениях текущей работы. Знак компоненты поля $Е_1$, полученный в данной работе, отрицательный. Если увеличить, вслед за Ферми, величину градиентного интеграла $Е_1$ (Ферми его увеличил в полтора раза), то можно получить отрицательный знак электрического импульса центрально симметричного взрыва плазмы, как это показано в публикации Менде.

И таким образом все сходится. Кроме одного. Кроме понимания того как на практике использовать потенциалы ЛВ для точного решения практических задач.
