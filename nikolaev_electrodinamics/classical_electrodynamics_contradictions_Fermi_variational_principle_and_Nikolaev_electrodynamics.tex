...
%==============================================================================
% ЧАСТЬ 6: СВЯЗЬ С ФЕРМИ И ПРИЛОЖЕНИЯ
%==============================================================================
\section{Связь продольной проницаемости с вариационным принципом Ферми}
\label{sec:fermi_connection}

В работе Ферми 1923 года «Разрешение существующего противоречия между электродинамической и релятивистской теориями электромагнитной массы» показано, что для корректного вычисления электромагнитной массы ускоряющегося заряда необходимо использовать вариацию типа В. Это приводит к модификации меры интегрирования в действии.
...
\label{eq:fermi_correction_full}
\end{equation}
...
\label{eq:measure_modification_full}
\end{equation}
...
\label{eq:lagrangian_eff_full}
\end{equation}

Сопоставляя коэффициенты при $\frac{\partial^2 \phi}{\partial t^2}$ в волновом уравнении (\ref{eq:phi_wave_final}) и в действии с поправкой Ферми, можно получить выражение для продольной проницаемости:

\begin{equation}
\mu_{\parallel} = \mu \cdot \lim_{V \to 0} \frac{\int_V \frac{\vec{a} \cdot \vec{r}}{c^2} \, dV}{\int_V dV}
\label{eq:mu_parallel_fermi_full}
\end{equation}

Таким образом, $\mu_{\parallel}$ физически интерпретируется как проявление искривления мировой трубки ускоряющегося заряда. В макроскопических уравнениях это выглядит как свойство вакуума (или среды), обладающего различной проницаемостью для поперечных ($\mu$) и продольных ($\mu_{\parallel}$) процессов.
