...
\end{array}\)
    ...
    So we can find expression (\ref{eq:9}) using the following summation:
...
\noindent
    ...
    are 0 in sum. And so any possible non zero result of expression (\ref{eq:9}) should be related with
remainder part:
...
as \(a\,k_m\,>>\,1\). But now we can investigate how the expression (\ref{eq:9}) will depends on \(a k_m\)
