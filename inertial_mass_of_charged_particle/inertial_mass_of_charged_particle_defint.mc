...
I1    (ra, theta_a, rq, theta_q) := defint(1/R(ra, theta_a, rq, theta_q, phi_q), phi_q, 0, 2 * %pi);
...
I2    (ra, theta_a, rq)          := defint(sin(theta_q) * Iphi(ra, theta_a, rq, theta_q), theta_q, 0, %pi);

rho_q (rho0, Rq, aq, r)          := rho0 / (1+exp((rq-Rq)/aq));
I3(rho0, Rq, aq, ra, theta_a) := defint(rho_q(rho0, Rq, aq, rq) * rq^2 * I2(ra, theta_a, rq), rq, 0, inf);

I4(rho0, Rq, aq, ra, theta_a) := 2 * %pi * I3(rho0, Rq, aq, ra, theta_a);
I5(rho0, Rq, aq, ra)          := defint(sin(theta_a) * I4(rho0, Rq, aq, ra, theta_a), theta_a, 0, %pi);
I6(rho0, Rq, aq)              := defint(rho_q(rho0, Rq, aq, ra) * ra^2 * I5(rho0, Rq, aq, ra), ra, 0, inf);
I6(1.0, 1.0, 1.0);