...
As a result, it has been found that the formula for the force per unit and for the energy density
difference per unit area is very similar to the formula of the classical Casimir effect,
...
\begin{multicols}{2}
    ...
    This expression obtained is clearly infinite, and to proceed with the
...
we will use the fact that the remainder term can also
...
\(a\,k_m\,>>\,1\). But now we can investigate how the expression (\ref{eq:12}) will depends on \(a k_m\).
...
obtained for the difference of specific energy density on its
...
of the V-shaped profile by using the Casimir formula, ``with the most
...
Thrust of the metal V-shaped profile is
\noindent
...
Thus, the formula for the thrust for the V-shaped profile is obtained basing on the
length of its wings.
...
\item
  ...
  the substance from which the V-shaped profile is made (by \(k_m\) value).
\end{itemize}
...
nanocells), it is important to note that for a V-shaped profile with a right angle
...
rectangular V-shaped profiles, it can be shown that the thrust of the panel consisting
...
combination of four V-shaped profiles where a half-angle is equal to
\(\alpha = {\pi}/{4}\). The thrust of the every V-shaped profile
\noindent
...
It should be noted, that this formula is obtained without taking
...
panel produced from acute-angled V-shaped profiles, but producing
panels from honeycombs seems to be technologically simpler than
from V-shaped profiles.
